...
export class WordProblem {
  ...
  getNumber(mathQuestion) {
    ...
    if (result === null) throw new ArgumentError("Expected a number, but didn't find one")
  ...
  getOperator(mathQuestion) {
    this.removeSpace(mathQuestion)
    const operators = { plus, minus, 'multiplied by': multipliedBy, 'divided by': dividedBy}
  ...
  tokenize() {
    if (!this.question.endsWith('?')) throw new ArgumentError("The question should end with '?'");
    if (!this.question.startsWith('What is ')) throw new ArgumentError("The question should start with 'What is'");
    let mathQuestion = {
