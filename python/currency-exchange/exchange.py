...
def exchangeable_value(
    ...
    """
    exchange_rate_with_spread = exchange_rate * (1 + spread * 0.01)
    return get_value_of_bills(
        denomination,
        get_number_of_bills(
            exchange_money(budget, exchange_rate_with_spread), denomination
        ),
    )
...
def non_exchangeable_value(
    ...
    return math.floor(
        get_change(exchange_money(budget, exchange_rate_with_spread), exchangeable)
    )
